...
The Builder Track is a comprehensive hands-on engineering curriculum for developers building decentralized applications, protocol smart contracts, and Web3 infrastructure.
It emphasizes production-grade practices, modern developer tooling (specifically Foundry), rigorous testing methodologies, and smart contract security.
...
- Connect frontend web applications to smart contracts using Viem, Wagmi, and wallet connection libraries.
...
3. **Key Management & Network Interactions**
   ...
   - Managing public and private JSON-RPC providers (Alchemy, Infura, QuickNode).

### Phase 2: Solidity Architecture & Low-Level Primitives

1. **Solidity Type System & Fundamentals**
   - Value types versus reference types (arrays, structs, mappings).
   - Visibility specifiers (`external`, `public`, `internal`, `private`) and mutability (`view`, `pure`, `payable`).
...
4. **Low-Level Calls & Address Interactions**
   - Sending Ether: `transfer` versus `send` versus `call{value: ...}("")`.
...
1. **ERC-20 Fungible Tokens**
   ...
   - OpenZeppelin standard implementations and permit signatures (EIP-2612).
...
3. **ERC-1155 Multi-Token Standard**
   - Batch transfers, mixed fungible/non-fungible balances, and gas savings.
...
1. **Solidity-Native Testing with Forge**
   ...
   - Forge cheatcodes: `prank`, `deal`, `warp`, `roll`, `expectRevert`, and `expectEmit`.
...
1. **Constant Product Automated Market Makers (AMM)**
...
2. **Staking & Reward Distribution Algorithms**
   - Implementing scalable reward distribution (Synthetix staking rewards algorithm).
...
1. **Modern Web3 Frontend Stack**
   - Connecting Vue or React frontends with Viem and Wagmi.
...
1. **Common Vulnerability Patterns**
   - Reentrancy attacks (single-function, cross-function, and read-only reentrancy).
